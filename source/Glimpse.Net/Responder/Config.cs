...
namespace Glimpse.Net.Responder
{
    ...
    public class Config : GlimpseResponder
    {
        ...
        public override void Respond(HttpApplication application, GlimpseConfiguration configuration)
        {
            ...
            response.Write(string.Format("</ol></li></ul><h2>Your Settings:</h2><p>This section tells you how Glimpse sees your requests.</p><ul><li>IP = {0}</li><li>glimpseState = <label for='gChk' id='glimpseState'>{1}</label></li></ul>", application.Request.ServerVariables["REMOTE_ADDR"], mode));
