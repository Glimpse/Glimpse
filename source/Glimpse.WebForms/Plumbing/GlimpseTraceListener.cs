    ...
    public class GlimpseTraceListener : TraceListener
    {
        private static Stopwatch FirstWatch
        {
            get
            {
                ...
                var firstWatch = store[Plugin.Trace.FirstWatchStoreKey] as Stopwatch;

                if (firstWatch != null) return firstWatch;

                store[Plugin.Trace.FirstWatchStoreKey] = firstWatch = new Stopwatch();
                firstWatch.Start();

                return firstWatch;
            }
            set { HttpContext.Current.Items[Plugin.Trace.FirstWatchStoreKey] = value; }
        }
        ...
        private static Stopwatch LastWatch
        {
            ...
            {
                var store = HttpContext.Current.Items;
                var lastWatch = store[Plugin.Trace.LastWatchStoreKey] as Stopwatch;

                if (lastWatch != null) return lastWatch;

                store[Plugin.Trace.LastWatchStoreKey] = lastWatch = new Stopwatch();
                lastWatch.Start();

                return lastWatch;
            }
            set { HttpContext.Current.Items[Plugin.Trace.LastWatchStoreKey] = value; }
        }
