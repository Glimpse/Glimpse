...
if (window.jQueryGlimpse) { (function ($) {
    ...
    $.extend($.glimpseProcessor, {
        ...
        build: function (data, level, forceFull, metadata, tolerance) {
            var that = this, result = '';

            if ($.isArray(data)) {
                if (metadata)
                    result = that.buildStructuredTable(data, level, forceFull, metadata, tolerance);
                else
                    result = that.buildCustomTable(data, level, forceFull, tolerance);
            }
            else if ($.isPlainObject(data))
                result = that.buildKeyValueTable(data, level, forceFull, tolerance);
            else if (level == 0) {
        ...
        buildKeyValueTable: function (data, level, forceFull, tolerance) {
            ...
            for (var key in data)
                html += '<tr class="' + (i++ % 2 ? 'odd' : 'even') + '"><th width="30%">' + $.glimpseContent.formatString(key) + '</th><td width="70%"> ' + that.build(data[key], level + 1, false, undefined, 1) + '</td></tr>';
            html += '</table>';
            return html;
        },
        buildCustomTable: function (data, level, forceFull, tolerance) {
            var that = this, limit = 3 * tolerance; 
            if (!forceFull && ((level > 0 && data.length > (limit + 1)) || level > 1))
            ...
            if ($.isArray(data[0])) {
                ...
                for (var i = 1; i < data.length; i++) {
                    ...
                    for (var x = 0; x < data[0].length; x++)
                        html += '<td>' + that.build(data[i][x], level + 1, false, undefined, 1) + '</td>';
                    html += '</tr>';
                }
                html += '</table>';
            }
                ...
                if (data.length > 1) {
                    ...
                    for (var i = 0; i < data.length; i++)
                        html += '<tr class="' + (i % 2 ? 'odd' : 'even') + '"><td>' + that.build(data[i], level + 1, false, undefined, 1) + '</td></tr>';
                    html += '</table>';
                }
                else
                    html = that.build(data[0], level + 1, false, undefined, 1)
            }
            return html;
        },
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var x = 0; x < metadata.length; x++) { 
                    ...
                    for (var y = 0; y < metadata[x].length; y++) {
                        ...
                        rowData += that.buildStructuredTableCell(data[i], metadataItem, level, newTolerance, cellType, i);
                    }
        ...
        buildStructuredTableCell : function(data, metadataItem, level, tolerance, cellType, rowIndex) {
            ...
            if ($.isArray(metadataItem.data)) {
                ...
                cellContent = that.build(cellContent, level + 1, metadataItem.forceFull, newMetadataItem, tolerance);
        ...
        newItemSpacer: function (currentRow, rowLimit, dataLength) { //TODO: this needs to be moved out 
            var html = '';
