...
if (window.jQueryGlimpse) { (function ($) {
    ...
    $.extend($.glimpseProcessor, {
        ...
        buildStructuredTable : function (data, level, forceFull, metadata) {
            ...
            var html = ''; 
            for (var i = 0; i < data.length; i++) {
                html += (i == 0) ? '<div class="glimpse-row-header-' + level + '">' : '<div class="' + (i % 2 ? 'odd' : 'even') + '">';
                html += '<table width="100%">';
                for (var x = 0; x < metadata.length; x++) {
                    html += '<tr>';
                    for (var y = 0; y < metadata[x].length; y++) {
                        var metadataItem = metadata[x][y], cellType = (metadataItem.key === true) ? 'th' : 'td';
                        html += '<' + cellType;
                        ...
                        if (metadataItem.className) { html += ' class="' + metadataItem.className + '"' };
                        if (metadataItem.align) { html += ' style="text-align:' + metadataItem.align + ';"' }; 
                        html += '>' + data[i][metadataItem.index] + '</' + cellType + '>';
                    }
                    html += '</tr>';
                }
                html += '</table></div>';
