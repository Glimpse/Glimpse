...
if (window.jQueryGlimpse) { (function ($) {
    ...
    $.extend($.glimpseProcessor, {
        ...
        applyPostRenderTransforms: function (scope) {
            ...
            $('.info, .warn, .error, .fail, .loading, .ms', scope).find('> td:first-child, > tr:first-child > td:first-child').not(':has(.icon)').prepend('<div class="icon"></div>');
        ...
        buildStructuredTable : function (data, level, forceFull, metadata) {
            ...
            var html = '<table>', isSingleRow = (metadata.length > 1), cellMasterClass = '', rowClass = ''; 
            for (var i = 0; i < data.length; i++) { 
                ...
                for (var x = 0; x < metadata.length; x++) {
                    ...
                    for (var y = 0; y < metadata[x].length; y++) {
                        var metadataItem = metadata[x][y], cellType = (i == 0 ? 'th' : 'td'), cellClass = cellMasterClass;
                        if (i > 0) {
