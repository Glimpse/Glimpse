...
var glimpseTimeline = function (scope, settings) { 
    var elements = {},
            ...
            var render = function () {
                ...
                processCategories = function () {
                    for (var categoryName in settings.category) {
                        ...
                        elements.summaryDescHolder.append('<div class="glimpse-tl-band"><div class="glimpse-tl-event glimpse-tl-event-' + category.event + '"></div>' + categoryName +'</div>'); 
        ...
        filter = function () {
            var zoom = function (persentLeft, persentRight) {  
                    var persentRightFromLeft = 100 - persentRight; 

                    for (var i = 0; i < settings.events.length; i += 1) {
                        var event = settings.events[i],
                            show = !(persentLeft > event.endPersent || persentRightFromLeft < event.startPersent);
