    ...
    $.extend($.glimpseSwitcher, {
        ...
        adjustLayout: function () {
            ...
            var dropFunction = function(scope) {
                ...
                $('.glimpse-drop', scope).mouseover(function() { $('.glimpse-drop-over', scope).fadeIn('fast'); });
            };
            //Setup environements
            if (environments) {
            ...
            //Setup correlation
            if (correlation) { 
                var urlHolder = $('.glimpse-title .glimpse-url', mainHolder), currentUrl = urlHolder.text(), correlationList = '<div>' + correlation.title + '</div>';
                for (var i = 0; i < correlation.legs.length; i++) {
                    var leg = correlation.legs[i];
                    correlationList += (leg.url == currentUrl ? currentUrl : '<a title="Go to '+ leg.url + '" href="#">'+ leg.url + '</a>') + ' (' + leg.method + ')<br />';
                }
                    
                urlHolder.html($('<span class="glimpse-drop">' + currentUrl + '<span class="glimpse-drop-arrow-holder"><span class="glimpse-drop-arrow"></span></span></span><div class="glimpse-drop-over">' + correlationList + '</div>'));
