...
namespace Glimpse.Core.Resource
{
    ...
    public class ConfigurationResource : IPrivilegedResource, IKey, IDependOnResources
    {
        ...
        {
            var glimpseConfigurationResourceName = "EmbeddedResources/" + InternalName + ".html";
            var glimpseConfigurationResourceStream = this.GetType().Assembly.GetManifestResourceStream(glimpseConfigurationResourceName);
            ...
            var glimpseConfigurationTemplateContent = new StreamReader(glimpseConfigurationResourceStream).ReadToEnd();
            var glimpseConfigurationTemplate = new Antlr4.StringTemplate.Template(new TemplateGroup('$', '$'), glimpseConfigurationTemplateContent);
            ...
            glimpseConfigurationTemplate.Add("tabsByPackage", GroupItemsByPackage(configuration.Tabs.OrderBy(x => x.Name), packages, createTabItemDisplay).ToArray());

            // Runtime Policies
            Func<IRuntimePolicy, object> createRuntimePolicyItemDisplay = registeredRuntimePolicy =>
                {
                    var warningMessage = registeredRuntimePolicy.GetType().FullName == "Glimpse.AspNet.Policy.LocalPolicy" ? "*This policy means that Glimpse won't run remotely.*" : string.Empty;
                    return new
                        {
            ...
            glimpseConfigurationTemplate.Add("runtimePoliciesByPackage", GroupItemsByPackage(configuration.RuntimePolicies.OrderBy(x => x.GetType().FullName), packages, createRuntimePolicyItemDisplay).ToArray());
            ...
            glimpseConfigurationTemplate.Add("inspectorsByPackage", GroupItemsByPackage(configuration.Inspectors.OrderBy(x => x.GetType().FullName), packages, createInspectorItemDisplay).ToArray());
            ...
            glimpseConfigurationTemplate.Add("resourcesByPackage", GroupItemsByPackage(configuration.Resources.OrderBy(x => x.Name), packages, createResourceItemDisplay).ToArray());
            ...
            glimpseConfigurationTemplate.Add("clientScriptsByPackage", GroupItemsByPackage(configuration.ClientScripts.OrderBy(x => x.GetType().FullName), packages, createClientScriptItemDisplay).ToArray());
            ...
            glimpseConfigurationTemplate.Add("registeredNuGetPackages",
            ...
            var nonProcessableAssemblies = nuGetPackageDiscoveryResult.NonProcessableAssemblies;
