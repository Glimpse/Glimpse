...
(function($, data, elements, util, pubsub) {
    var wireListeners = function () {
        ...
        generateHtml = function(pluginDataSet) {
            ...
            for (var key in pluginDataSet) {
                var pluginData = pluginDataSet[key],
                    ...
                    item = '<li class="glimpse-tab glimpse-tabitem-' + key + disabled + '" data-glimpseKey="' + key + '">' + pluginData.name + '</li>';
        ...
        render = function() {
            ...
            tabPermanentHolder.html(tabHtml.permanent);
...
(function($, pubsub, util, elements, data, renderEngine) {
    var isActive = true,
        ...
        wireListeners = function () {
            ...
            panel.find('.glimpse-head-message a').live('click', function() { pubsub.publish('action.data.context.reset', 'ajax'); }); //TODO: Need to check how this wires up
        },
        ...
        retreieveSummaryData = function() { 
            ...
            notice.prePoll(); 
            $.ajax({
                url: util.uriTemplate(data.currentMetadata().resources.glimpse_ajax, { 'parentRequestId': data.currentData().requestId, 'ajaxResults': true }),
                type: 'GET',
                contentType: 'application/json',
                complete : function(jqXHR, textStatus) {
                    if (!isActive) 
